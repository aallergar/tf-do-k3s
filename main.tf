...
resource "digitalocean_loadbalancer" "k3s-server" {
  ...
  # k3s API server — used by kubectl and by joining nodes (server + agent)
  forwarding_rule {
    entry_port     = 6443
    entry_protocol = "tcp"
  ...
  # Health check on the k3s API port.
  # RKE2 used port 9345 (supervisor); k3s exposes everything on 6443.
  healthcheck {
    port     = 6443
    protocol = "tcp"
  }
...
output "k3s_server-nodes-address" {
  description = "k3s server nodes IP"
  value       = length(digitalocean_droplet.server-node[*].ipv4_address) > 0 ? digitalocean_droplet.server-node[*].ipv4_address : null
}
output "k3s_agent-nodes-address" {
  description = "k3s agent nodes IP"
